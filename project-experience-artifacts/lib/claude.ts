		...
		const response = await client.messages
			...
			.catch(async (err) => {
				if (err instanceof Anthropic.APIError) {
					...
					if (err.status === 400) {
						throw new Error(
							`Claude API Bad Request (400): ${err.message}. Check if model '${model}' is valid and available.`
						);
					} else if (err.status === 401) {
						throw new Error(
							`Claude API Unauthorized (401): ${err.message}. Check your ANTHROPIC_API_KEY.`
						);
					} else if (err.status === 429) {
						throw new Error(
							`Claude API Rate Limited (429): ${err.message}. Please try again later.`
						);
					} else {
