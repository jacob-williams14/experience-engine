...
async function generateBio(
	...
	try {
		...
		throw new Error(
			"generateBio.ts is not yet implemented - this is scaffolding only"
		);
	} catch (error) {
...
async function main() {
	...
	if (args.length < 1) {
		console.error(
			"Usage: bun run scripts/generateBio.ts <developer-name> [options]"
		);
		console.error("");
		...
		console.error(
			'  bun run scripts/generateBio.ts "Jacob Williams" --career-level "mid-level"'
		);
		process.exit(1);
	}
