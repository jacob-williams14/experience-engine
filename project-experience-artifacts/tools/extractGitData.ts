 ...
 */
function filterCommitsByDeveloper(commits: GitCommit[], developer: Developer): GitCommit[] {
  const namesToMatch = [
...
async function extractGitData(
  ...
  try {
    ...
    const entries = fileContent.split(/(?=^commit )/gm).filter(entry => entry.trim());
    ...
    console.log(`🎯 Found ${developerCommits.length} commits by ${developerName}`);
    ...
    const commitsToProcess = developerCommits.slice(0, TOOL_CONFIG.MAX_COMMITS_TO_PROCESS);
    ...
    if (options.saveToFile) {
      const outputFile = `${TOOL_CONFIG.OUTPUT_DIR}/${developer.name.toLowerCase().replace(/\s+/g, '-')}-${project.slug}-git-data.json`;
...
async function main() {
  ...
  if (args.length < 3) {
    console.error('Usage: bun run tools/extractGitData.ts <git-log-file> <developer-name> <project-name> [options]');
    console.error('');
    ...
    console.error('  --save                    Save processed data to JSON file');
    console.error('');
    console.error('Example:');
    console.error('  bun run tools/extractGitData.ts compass-git-log.txt "Jacob Williams" "Compass Platform" --email jacob@example.com --save');
    process.exit(1);
  }
  ...
  try {
    const result = await extractGitData(filePath!, developerName!, projectName!, options);
    
    if (result.success) {
      console.log('\n✅ Git data extraction completed successfully!');
      console.log(`📊 Processed ${result.metadata.recordsProcessed} commits in ${result.metadata.processingTime}ms`);
